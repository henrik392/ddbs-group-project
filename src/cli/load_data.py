...
def bulk_load(sql_dir: Path):
    ...
    dbms3_conn = "postgresql://ddbs:ddbs@localhost:5436/ddbs3"
    ...
    dbms3_files = ["user_dbms3.sql", "read_dbms3.sql"]
    ...
    # Load into DBMS3
    print("\nLoading data into DBMS3 (Shanghai)...")
    with psycopg.connect(dbms3_conn) as conn:
        for filename in dbms3_files:
            file_path = sql_dir / filename
            if not file_path.exists():
                print(f"  ⚠ Skipping {filename} (not found)")
                continue
    ...
    print("  DBMS3 (Shanghai): Shanghai users + Shanghai reads (no articles)")
...
def verify():
    ...
    for region, count in regions:
        print(f"    - {region}: {count}")
    print(f"  Articles: {article_count} total (should be 0)")
    print(f"  Reads: {read_count} total")
